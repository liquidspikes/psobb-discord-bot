...
- **Drop mechanics (slash path):**
  ...
  - **Stat shifts / "instability"** — a syndicate shift (`incrementDropGuesses` reaching its threshold) or an individual shift (a player exhausting their attempt budget, `shiftActiveDropStats`). Both clear the transient per-guess phase messages and then post a **persistent, public "⚠️ Instability Detected" announcement** (visible to everyone, in both live and test mode; the announcement is not tracked in `phaseMessageIds`, so later clears don't remove it).
  - **Despawn pulse** — each guess calls `pulseDespawnTime` (adds time, capped); the watcher expires the drop when `despawn_time` passes.
- **Tokens carry no weapon** — only the five guaranteed percentages. A win mints a token (`createToken`) and posts a single **public** win embed (the deferred ephemeral is deleted, so the winner doesn't get a duplicate private copy). **Redemption happens on the website** (https://psobb.io/, while online in-game, combining up to 3 tokens); the bot never drops the item. *(The old per-win and per-guess hint DMs were removed.)*
- **Ephemeral cleanup on game end** — each non-win `/guess` interaction is tracked in `guessInteractions`; when the game ends (win / `!tekker end` / despawn), `clearGuessEphemerals()` deletes those private result messages. Ephemerals aren't channel messages so the channel wipe can't touch them, and Discord only allows deleting them within **~15 min** of the interaction — older ones are silently skipped. The list resets when a new drop is announced.
